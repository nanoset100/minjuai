...
POST /api/letters            — 편지 제출 + SendGrid 발송
...
class LetterRequest(BaseModel):
    ...
    content: str = Field(..., min_length=10, description="편지 내용")
...
def submit_letter(req: LetterRequest):
    ...
    if result.get("blocked"):
        raise HTTPException(
            ...
            detail={
                "blocked": True,
                "reason": result.get("reason", "편지 발송 기준에 맞지 않습니다"),
            }
        )

    if not result.get("success"):
        raise HTTPException(status_code=500, detail="편지 발송 중 오류가 발생했습니다")
    ...
    return {
        ...
        "message": f"{result['member_name']} 의원실에 편지가 전달됐습니다",
    }
